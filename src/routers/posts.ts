...
import { getPosts, getPost, createPost, updatePost, deletePost } from '../application/post/usecase/'
...
router.post('/', verifyToken, async (req, res) => {
  try {
    ...
    const newPost = await createPost({authorId: req.user.id, title, content, imagePath})
...
router.put('/:id', verifyToken, async (req: Request, res) => {
  try {
    const {title, content, imagePath} = req.body
    const newPost = await updatePost({postId: Number(req.params.id), title, content, imagePath})
    res.status(201).json(newPost)
  } catch (error) {
    res.status(500).json(error)
  }
